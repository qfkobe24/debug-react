...
export function scheduleUpdateOnFiber(
  ...
  if (expirationTime === Sync) {
    if (
      ...
      // 同步任务入口，没有通过 scheduler，都是函数的直接调用
      performSyncWorkOnRoot(root);
    } else {
      // 后续的更新通过 scheduler 调度实现更新
...
// 与performSyncWorkOnRoot 的区别，是异步可中断
...
function performSyncWorkOnRoot(root) {
  ...
  if (root !== workInProgressRoot || expirationTime !== renderExpirationTime) {
    // 构建 workInProgressFiber 树以及 rootFiber
    prepareFreshStack(root, expirationTime);
    startWorkOnPendingInteractions(root, expirationTime);
  }
...
// 构建 workIngressFiber 树以及 rootFiber
...
function workLoopSync() {
  ...
  while (workInProgress !== null) {
    console.log('%cMyProject%cline:1476%cworkInProgress.stateNode', 'color:#fff;background:#ee6f57;padding:3px;border-radius:2px', 'color:#fff;background:#1f3c88;padding:3px;border-radius:2px', 'color:#fff;background:rgb(248, 147, 29);padding:3px;border-radius:2px', workInProgress.stateNode)
...
function performUnitOfWork(unitOfWork: Fiber): Fiber | null {
  ...
  if (enableProfilerTimer && (unitOfWork.mode & ProfileMode) !== NoMode) {
    ...
    // console.log('beginWork', unitOfWork.stateNode)
  ...
  if (next === null) {
    ...
    // 子到父，归的阶段
    // console.log('completeWork', unitOfWork.stateNode)
...
function commitRoot(root) {
  ...
  // ImmediatePriority，优先级99，最高优先级
  runWithPriority(
    ImmediatePriority,
    commitRootImpl.bind(null, root, renderPriorityLevel),
  );
  console.log('commitRoot 进入render阶段');
  return null;
}
...
function commitLayoutEffects(
  ...
  while (nextEffect !== null) {
    setCurrentDebugFiberInDEV(nextEffect);
    // 此时 effectTag 已经被重置为1，表示 DOM 操作已经完成
    const effectTag = nextEffect.effectTag;
